    function submitForm()
  {
   ...
   {
	   const userMessageLength = $("#message").val().trim().length; 
	   const userContactDataLength = $("#user-contact-data").val().trim().length; 
	   
	   if(userMessageLength < 1001 && userContactDataLength < 31 )
	   {
		   $("#anti-spam-tool").css("display","block");
	   }
	   else
	   {
		 alert("Należy podać treść wiadomości nie większą niż 1000 znaków. Namiary nie mogą przekroczyć 30 znaków"); 
	   }
	      
   }
   
  
      $("#send-message-button").click(initiateInsertion);
	});